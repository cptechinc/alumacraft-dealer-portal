	...
	if (isset($_GET['search'])) {	
			...
			switch ($role_type) {
				...
				case 'SADMIN':
					if (isset($_GET['location'])) {
						...
						$sql = get_boat_inventory_allsearch($dplus_custs, $showonpage, $this_page, $keyword, $matchingboats, $reg, true);
					}
					break;
			}
		...
		switch ($role_type) {
			...
			case 'SREP':
				if (isset($_GET['location'])) {
					//$dealerid comes from the role-type-logic script
					$boats = get_cust_boat_inventory($dealerid, $showonpage, $this_page, 'N', false);
					$sql = get_cust_boat_inventory($dealerid, $showonpage, $this_page, $reg, false);
				} else {
