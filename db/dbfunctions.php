...
/* =============================================================
 ...
 ============================================================ */
	function get_orders($custlist, $limit, $page, $approved, $boatsonly, $debug) { 
		...
		if ($page > 1) { $start_point = ($page * $limit) - $limit; } else if ($page == 1) { $start_point = 0; } else { $start_point = 0; }
		if ($boatsonly) {
			$sql = "SELECT OedtDesc AS descr, SO_DETAIL.InitItemNbr, SO_HEADER.* FROM SO_HEADER JOIN SO_DETAIL ON SO_DETAIL.OehdNbr = SO_HEADER.OehdNbr JOIN boat_master ON SO_DETAIL.InitItemNbr = ItemNbr WHERE ArcuCustId IN ($custlist) AND OehdUserCode3 = '$approved' LIMIT ".$start_point.",".$limit;
		} else {
	...
	function get_orders_search($custlist, $searchcol, $keyword, $limit, $page, $approved, $boatsonly, $debug) { 
		...
		if ($page > 1) { $start_point = ($page * $limit) - $limit; } else if ($page == 1) { $start_point = 0; } else { $start_point = 0; }
		//if ($boatsonly) {
			$sql = "SELECT OedtDesc AS descr, SO_HEADER.* FROM SO_HEADER JOIN SO_DETAIL ON SO_DETAIL.OehdNbr = SO_HEADER.OehdNbr JOIN boat_master ON SO_DETAIL.InitItemNbr = ItemNbr WHERE ArcuCustId IN ($custlist) AND OehdUserCode3 = '$approved' AND UCASE($searchcol) LIKE UCASE('$search') LIMIT ".$start_point.",".$limit;
		/*} else {
	...
	function get_orders_searchall($custlist, $keyword, $limit, $page, $approved, $boatsonly, $debug) { 
		...
		if ($page > 1) { $start_point = ($page * $limit) - $limit; } else if ($page == 1) { $start_point = 0; } else { $start_point = 0; }
		//if ($boatsonly) {
			$sql = "SELECT OedtDesc AS descr, SO_HEADER.* FROM SO_HEADER JOIN SO_DETAIL ON SO_DETAIL.OehdNbr = SO_HEADER.OehdNbr JOIN boat_master ON SO_DETAIL.InitItemNbr = ItemNbr WHERE ArcuCustId IN ($custlist) AND OehdUserCode3 = '$approved' AND CONCAT(SO_DETAIL.OehdNbr, ' ', OedtDesc, ' ', InitItemNbr, ' ', ArcuCustId, ' ', ArstShipId, ' ', OehdStLastName, ' ', OehdStZipCode, ' ', OehdCustPo, ' ', OehdOrdrDate, ' ', OehdArrvDate, ' ', OehdUserCode2) LIKE '$search' LIMIT ".$start_point.",".$limit;
		//} else {
			//$sql = "SELECT OedtDesc AS descr, SO_HEADER.* FROM SO_HEADER JOIN SO_DETAIL ON SO_DETAIL.OehdNbr = SO_HEADER.OehdNbr WHERE ArcuCustId IN ($custlist) AND OehdUserCode3 = '$approved' AND CONCAT(SO_DETAIL.OehdNbr, ' ', OedtDesc, ' ', InitItemNbr, ' ', ArcuCustId, ' ', ArstShipId, ' ', OehdStLastName, ' ', OehdStZipCode, ' ', OehdCustPo, ' ', OehdOrdrDate, ' ', OehdArrvDate, ' ', OehdUserCode2) LIKE '$search' LIMIT ".$start_point.",".$limit;
		//}
	...
	function get_orders_datesearch($custlist, $datecol, $datefrom, $datethrough, $limit, $page, $approved, $boatsonly, $debug) { 
		...
		if ($page > 1) { $start_point = ($page * $limit) - $limit; } else if ($page == 1) { $start_point = 0; } else { $start_point = 0; }
		//if ($boatsonly) {
			$sql = "SELECT OedtDesc AS descr, SO_HEADER.* FROM SO_HEADER JOIN SO_DETAIL ON SO_DETAIL.OehdNbr = SO_HEADER.OehdNbr JOIN boat_master ON SO_DETAIL.InitItemNbr = ItemNbr WHERE ArcuCustId IN ($custlist) AND OehdUserCode3 = '$approved' AND (STR_TO_DATE($datecol, '%Y%m%d') BETWEEN STR_TO_DATE('$datefrom', '%Y%m%d') AND STR_TO_DATE('$datethrough', '%Y%m%d')) LIMIT ".$start_point.",".$limit;
		//} else {
		//	$sql = "SELECT OedtDesc AS descr, SO_HEADER.* FROM SO_HEADER WHERE ArcuCustId IN ($custlist) AND OehdUserCode3 = '$approved' AND (STR_TO_DATE($datecol, '%Y%m%d') BETWEEN STR_TO_DATE('$datefrom', '%Y%m%d') AND STR_TO_DATE('$datethrough', '%Y%m%d')) LIMIT ".$start_point.",".$limit;
		//}
	...
	function get_cust_orders($custid, $limit, $page, $approved, $boatsonly, $debug) { 
		...
		if ($page > 1) { $start_point = ($page * $limit) - $limit; } else if ($page == 1) { $start_point = 0; } else { $start_point = 0; }
			...
			$sql = "SELECT OedtDesc AS descr,SO_DETAIL.InitItemNbr,  SO_HEADER.* FROM SO_HEADER JOIN SO_DETAIL ON SO_DETAIL.OehdNbr = SO_HEADER.OehdNbr JOIN boat_master ON SO_DETAIL.InitItemNbr = ItemNbr WHERE ArcuCustId = '$custid' AND OehdUserCode3 = '$approved' LIMIT ".$start_point.",".$limit;
	...
	function get_cust_orders_search($custid, $searchcol, $keyword, $limit, $page, $approved, $boatsonly, $debug) { 
		...
		if ($page > 1) { $start_point = ($page * $limit) - $limit; } else if ($page == 1) { $start_point = 0; } else { $start_point = 0; }
		if ($boatsonly) {
			$sql = "SELECT OedtDesc AS descr, SO_HEADER.* FROM SO_HEADER JOIN SO_DETAIL ON SO_DETAIL.OehdNbr = SO_HEADER.OehdNbr JOIN boat_master ON SO_DETAIL.InitItemNbr = ItemNbr WHERE ArcuCustId = '$custid' AND OehdUserCode3 = '$approved' AND UCASE($searchcol) LIKE UCASE('$search') LIMIT ".$start_point.",".$limit;
		} else {
			$sql = "SELECT OedtDesc AS descr, SO_HEADER.* FROM SO_HEADER JOIN SO_DETAIL ON SO_DETAIL.OehdNbr = SO_HEADER.OehdNbr WHERE ArcuCustId = '$custid' AND OehdUserCode3 = '$approved' AND UCASE($searchcol) LIKE UCASE('$search') LIMIT ".$start_point.",".$limit;
		}
	...
	function get_cust_orders_datesearch($custid, $datecol, $datefrom, $datethrough, $limit, $page, $approved, $boatsonly, $debug) { 
		...
		if ($page > 1) { $start_point = ($page * $limit) - $limit; } else if ($page == 1) { $start_point = 0; } else { $start_point = 0; }
		if ($boatsonly) {
			$sql = "SELECT OedtDesc AS descr, SO_HEADER.* FROM SO_HEADER JOIN SO_DETAIL ON SO_DETAIL.OehdNbr = SO_HEADER.OehdNbr JOIN boat_master ON SO_DETAIL.InitItemNbr = ItemNbr WHERE ArcuCustId = '$custid' AND OehdUserCode3 = '$approved' AND (STR_TO_DATE($datecol, '%Y%m%d') BETWEEN STR_TO_DATE('$datefrom', '%Y%m%d') AND STR_TO_DATE('$datethrough', '%Y%m%d')) LIMIT ".$start_point.",".$limit;
		} else {
	...
	function get_cust_orders_searchall($custid, $keyword, $matchingboats, $limit, $page, $approved, $boatsonly, $debug) { 
		...
		if ($page > 1) { $start_point = ($page * $limit) - $limit; } else if ($page == 1) { $start_point = 0; } else { $start_point = 0; }
		if ($boatsonly) {
			$sql = "SELECT OedtDesc AS descr, SO_HEADER.* FROM SO_HEADER JOIN SO_DETAIL ON SO_DETAIL.OehdNbr = SO_HEADER.OehdNbr JOIN boat_master ON SO_DETAIL.InitItemNbr = ItemNbr WHERE ArcuCustId = '$custid' AND OehdUserCode3 = '$approved' AND InitItemNbr IN ($matchingboats) AND CONCAT(SO_DETAIL.OehdNbr, ' ', OedtDesc, ' ', InitItemNbr, ' ', ArcuCustId, ' ', ArstShipId, ' ', OehdStLastName, ' ', OehdStZipCode, ' ', OehdCustPo, ' ', OehdOrdrDate, ' ', OehdArrvDate) LIKE '$search' LIMIT ".$start_point.",".$limit;
		} else {
			$sql = "SELECT OedtDesc AS descr, SO_HEADER.* FROM SO_HEADER JOIN SO_DETAIL ON SO_DETAIL.OehdNbr = SO_HEADER.OehdNbr WHERE ArcuCustId = '$custid' AND OehdUserCode3 = '$approved' AND CONCAT(SO_DETAIL.OehdNbr, ' ', OedtDesc, ' ', InitItemNbr, ' ', ArcuCustId, ' ', ArstShipId, ' ', OehdStLastName, ' ', OehdStZipCode, ' ', OehdCustPo, ' ', OehdOrdrDate, ' ', OehdArrvDate) LIKE '$search' LIMIT ".$start_point.",".$limit;
		}
	...
	function get_wip_complete_date($ordn, $itemid, $debug) {
		global $db;
		$sql = "SELECT WIP_HEADER.WiphCmpltdDate FROM SO_HEADER JOIN SO_DETAIL ON SO_HEADER.OehdNbr = SO_DETAIL.OehdNbr JOIN WIP_HEADER ON SO_DETAIL.InitItemNbr = WIP_HEADER.InitItemNbr AND SO_DETAIL.OehdNbr LIKE CONCAT(WIP_HEADER.OehdNbr, '%') WHERE SO_HEADER.OehdNbr = '$ordn' AND SO_DETAIL.InitItemNbr = '$itemid'";
		if ($debug) {
			return $sql;
		} else {
			$results = $db->query($sql);
	...
	function is_in_inv_lot($ordn, $itemid, $debug) {
		global $db;
		$sql = "SELECT COUNT(*) FROM INV_INV_LOT JOIN SERIAL_MAST ON INV_INV_LOT.InltLotSer = SERIAL_MAST.SermSerNbr  WHERE INV_INV_LOT.InltOnHand != '0' AND INV_INV_LOT.InitItemNbr = '$itemid' AND '$ordn' LIKE CONCAT(SermAcAllocOrdr, '%') ";
		if ($debug) {
			return $sql;
		} else {
			$results = $db->query($sql);
